...
def unpack_batch_a2c(batch, net, last_val_gamma, device="cpu"):
    ...
    for idx, exp in enumerate(batch):
        ...
        # if it's None, then last_state is a terminal state, has zero reward.
    ...
    if not_done_idx:
        ...
        # if done(s1->s2, s2 is a terminal state), then value of Q(s,a) is just the immediate reward, if not done, Q(s,a) = immediate reward + gamma * V(s2)
        rewards_np[not_done_idx] += last_val_gamma * last_vals_np

    # s1--a-->s2: ref_vals_v is estimated value of Q(s,a) 
    ref_vals_v = torch.FloatTensor(rewards_np).to(device)
    return states_v, actions_v, ref_vals_v


@torch.no_grad()
def unpack_batch_sac(batch, val_net, twinq_net, policy_net, gamma: float, ent_alpha: float, device="gpu"):
    """
    ...
    # act_dist.log_prob(acts_v): under the act distribution, the probability density of acts_v
